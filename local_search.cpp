...
void allocation(){
	...
	for(int i = 1;i <= n;i++)if(!visited[i])split_nodes.push_back(i);
	for(int j = 1;j <= n;j++) if(!cand[j].empty()) split_nodes.push_back(j);
	sort(split_nodes.begin(),split_nodes.end());
	split_nodes.erase(unique(split_nodes.begin(),split_nodes.end()),split_nodes.end());
	vector<int> choosen;
	for(int i = 1;i <= 100;i++){
...
signed main(){
	...
	read();
//	freopen("Rios_Mercado//Rios_Mercado//input//2DU60-05-1.dat","r",stdin);
	input();
