...
void allocation(){
	...
	auto collect = [&](const vector<double> &xx){
		for(int j = 1;j <= n;j++){
			...
			// cand[j] lưu những centroid có khả năng % 
		}
	};
	...
	for(int j = 1;j <= n;j++){
		...
		// cand[j] . 2 tiêu chí chọn 2 centroid khác nhau
	...
	auto within_balance = [&](int k,int j,int dir){
		// dir = {-1,1} -> ktra xem có cân bằng nếu thêm hoặc bớt j từ k hay không 
		for(int a = 0;a < 2;a++){
	...
	auto infeas_after_add = [&](int k,int j){
		// tính toán tổng vi phạm thay đổi nếu thêm j vào k 
		double total = 0;
		for(int a = 0;a < 2;a++){
	...
	for(int jj : split_nodes){
		...
		// ưu tiên tổng khoảng cách trước 
		for(int k = 1;k <= p;k++){
		...
		if(!picked){
			// ưu tiên mức độ chênh lệch đơn hàng, khách hàng 
...
void local_search(double gamma = 0.5,int limit_moves = 10000){
	rebuild_node_lists();
